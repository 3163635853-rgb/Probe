...
type Analysis = { uuid: string; overall_score: number; delivery_metrics: Record<string, number>; visual_metrics: Record<string, number | boolean> };
type Review = { uuid: string; session_uuid: string; status: string; rating?: number; comments: string };
...
function CareerSection() {
  ...
  async function pickResume() {
    const picked = await DocumentPicker.getDocumentAsync({ type: ["application/pdf", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain", "text/markdown"], copyToCacheDirectory: true });
...
function PracticeSection() {
  ...
  const [selected, setSelected] = useState<Drill | null>(null); const [answer, setAnswer] = useState(""); const [result, setResult] = useState<Optimized | null>(null); const [loading, setLoading] = useState(false);
  async function optimize() { if (!selected || !answer.trim()) return; setLoading(true); try { const data = await fetchAPI<Optimized>("/practice/optimize", { method: "POST", body: JSON.stringify({ question: selected.prompt, answer, evaluation: { suggestion: `强化${selected.dimension}` } }) }); setResult(data); } catch (error) { showError(error); } finally { setLoading(false); } }
  return <View className="gap-4"><Card title="弱项专项训练"><View className="flex-row flex-wrap gap-2">{drills?.map((item) => <TouchableOpacity key={item.code} onPress={() => { setSelected(item); setAnswer(""); setResult(null); }} className={`rounded-full px-3 py-2 ${selected?.code === item.code ? "bg-primary" : "bg-secondary"}`}><Text className={`text-xs font-semibold ${selected?.code === item.code ? "text-white" : "text-foreground"}`}>{item.name} · {item.duration_min}m</Text></TouchableOpacity>)}</View></Card>{selected && <Card title={selected.name}><Text className="rounded-xl bg-secondary p-3 text-sm leading-6 text-foreground">{selected.prompt}</Text><Field value={answer} onChangeText={setAnswer} placeholder="完整回答一次…" multiline /><PrimaryButton label={loading ? "优化中…" : "生成结构版 / 60 秒版 / STAR 版"} onPress={optimize} disabled={loading || !answer.trim()} />{result && <View className="gap-3"><AnswerVersion title="60 秒版" text={result.concise} /><AnswerVersion title="STAR 版" text={result.star} /></View>}</Card>}</View>;
}

function VideoSection() {
  const { data: history, refetch } = useFetch<Analysis[]>("/video/analyses"); const [transcript, setTranscript] = useState(""); const [loading, setLoading] = useState(false); const [analysis, setAnalysis] = useState<Analysis | null>(null);
  async function analyzeAsset(asset: { uri: string; name: string; type: string }) { const form = new FormData(); form.append("file", { uri: asset.uri, name: asset.name, type: asset.type } as unknown as Blob); form.append("transcript", transcript); setLoading(true); try { const data = await uploadFile<Analysis>("/video/analyze", form); setAnalysis(data); Burnt.toast({ title: "表达分析完成", preset: "done" }); refetch(); } catch (error) { showError(error); } finally { setLoading(false); } }
  ...
  return <View className="gap-4"><Card title="录像表达分析"><Text className="text-sm leading-6 text-muted-foreground">移动端可从相册或文件中选择录像；Web 训练室支持直接调用摄像头录制。</Text><Field value={transcript} onChangeText={setTranscript} placeholder="可选：粘贴转写文本" multiline /><View className="flex-row gap-2"><View className="flex-1"><PrimaryButton label={loading ? "分析中…" : "直接录制"} icon={<Video size={16} color="white" />} onPress={recordVideo} disabled={loading} /></View><View className="flex-1"><PrimaryButton label="选择录像" onPress={pickVideo} disabled={loading} /></View></View></Card>{current && <Card title={`表达表现 ${current.overall_score}/100`}><MetricRow label="语速" value={`${current.delivery_metrics.words_per_minute || 0} 字/分`} /><MetricRow label="填充词" value={`${current.delivery_metrics.filler_count || 0} 次`} /><MetricRow label="重复表达" value={`${current.delivery_metrics.repetition_count || 0} 处`} /><MetricRow label="面部出现" value={current.visual_metrics.available ? `${Math.round(Number(current.visual_metrics.face_presence_ratio || 0) * 100)}%` : "未分析"} /></Card>}</View>;
}

function CoachSection() {
  const { data: history } = useFetch<PaginatedData<InterviewHistoryItem>>("/interview/history?page=1&page_size=50"); const { data: reviews, refetch } = useFetch<Review[]>("/coach/reviews"); const sessions = history?.items.filter((item) => item.status === "completed") || []; const [selected, setSelected] = useState(""); const [focus, setFocus] = useState(""); const [loading, setLoading] = useState(false);
  async function submit() { if (!selected) return; setLoading(true); try { await fetchAPI("/coach/reviews", { method: "POST", body: JSON.stringify({ session_uuid: selected, focus }) }); Burnt.toast({ title: "已提交真人点评", preset: "done" }); setFocus(""); refetch(); } catch (error) { showError(error); } finally { setLoading(false); } }
  return <View className="gap-4"><Card title="选择面试报告">{sessions.map((item) => <TouchableOpacity key={item.session_uuid} onPress={() => setSelected(item.session_uuid)} className={`mb-2 rounded-xl border p-3 ${selected === item.session_uuid ? "border-primary bg-accent" : "border-border"}`}><Text className="font-semibold text-foreground">{item.position || "通用岗位"} · {item.final_score || 0} 分</Text><Text className="mt-1 text-xs text-muted-foreground">{new Date(item.started_at).toLocaleDateString("zh-CN")}</Text></TouchableOpacity>)}<Field value={focus} onChangeText={setFocus} placeholder="希望教练重点点评…" multiline /><PrimaryButton label={loading ? "提交中…" : "提交点评申请"} onPress={submit} disabled={!selected || loading} /></Card><Card title="点评进度">{reviews?.map((item) => <View key={item.uuid} className="mb-2 rounded-xl bg-secondary p-3"><View className="flex-row justify-between"><Text className="font-semibold text-foreground">{item.session_uuid.slice(0, 8)}</Text><Text className="text-xs text-primary">{item.status}</Text></View>{item.comments ? <Text className="mt-2 text-sm leading-6 text-foreground">{item.comments}</Text> : null}</View>)}</Card></View>;
}
