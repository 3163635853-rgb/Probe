...
- 开始面试时可绑定简历；Planner 和出题器会读取简历、故事、目标公司、面试轮次、面试官角色与训练重点。
...
- 指标包括回答时长、语速、填充词、长句、重复表达、停顿估计和估算静默时长。
- OpenCV 抽帧提供面部出现、眼部可见和面部居中比例；这些指标仅用于训练参考，不宣称进行情绪或心理诊断。
- 原录像可鉴权回放。

### 真人教练

- 用户可把已完成面试提交到教练队列并指定点评重点。
- 教练/管理员可领取任务、给出 1–10 分、长评和逐题批注。
- 完成后写入站内通知并尝试推送。

### 企业训练空间

- 组织与成员管理，支持 owner/admin/coach/member 角色。
- 自定义题库、自定义 Rubric 与自动归一化权重。
- 90 天团队看板、成员训练次数、均分和能力维度聚合。
- SSO 提供方、企业域名、数据保留天数和成员导出策略配置。
- 审计日志记录组织、成员、题库、Rubric、设置与导出操作。
- 组织数据 JSON 导出。
...
迁移：`c42f91a7e6d4_add_advanced_training_features.py`。
